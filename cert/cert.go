...
func (m *mkcert) makeCert(hosts []string) {
	...
	tpl := &x509.Certificate{
		...
		Subject: pkix.Name{
			Country:            []string{"UN"},
			Organization:       []string{"OpenNHP"},
			OrganizationalUnit: []string{"Root CA"},
			Locality:           []string{"Invisible"},
			Province:           []string{"Web"},
			CommonName:         "OpenNHP Root CA",
		},
		Issuer: pkix.Name{
			Country:            []string{"UN"},
			Organization:       []string{"OpenNHP"},
	...
	if !m.pkcs12 {
		...
		pfxData, err := pkcs12.Encode(rand.Reader, priv, domainCert, []*x509.Certificate{m.caCert}, DefaultPassword)
	...
	if !m.pkcs12 {
		...
		log.Printf("\nThe legacy PKCS#12 encryption password is the often hardcoded default \"%s\" \n\n", DefaultPassword)
	}
...
func (m *mkcert) fileNames(hosts []string) (certFile, keyFile, p12File string) {
	...
	keyFile = filepath.Join(m.CAROOT, defaultName+"-key.pem")
	if m.keyFile != "" {
		keyFile = m.keyFile
	}
...
func (m *mkcert) loadCA(createCA bool) error {
	caPath := filepath.Join(m.CAROOT, rootName)
	if !pathExists(caPath) {
		if !createCA {
			errMsg := fmt.Sprintf("The root CA file %s does not exist", caPath)
	...
	if !pathExists(filepath.Join(m.CAROOT, rootKeyName)) {
		// keyless mode, where only -install works
		return nil
	}
...
func (m *mkcert) caUniqueName() string {
	return "OpenNHP Root CA " + m.caCert.SerialNumber.String()
}
