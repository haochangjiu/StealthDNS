...
var (
	...
	errLoadConfig = fmt.Errorf("nhp-agent config load error")
)
...
func (a *UdpAgent) loadAgentConfig() error {
	// config.toml
	fileName := filepath.Join(common.ExeDirPath, "etc", "agent.toml")
	if err := a.updateAgentConfig(fileName); err != nil {
		// report base config error
		return err
	}

	baseConfigWatch = utils.WatchFile(fileName, func() {
		log.Info("agent base config: %s has been updated", fileName)
		a.updateAgentConfig(fileName)
	})
	return nil
}
...
func (a *UdpAgent) loadPeers() error {
	// server.toml
	fileName := filepath.Join(common.ExeDirPath, "etc", "server.toml")
...
func (a *UdpAgent) updateAgentConfig(file string) (err error) {
